# Python - HW6_Video Analysis - Chia-Yu Chen
...
# get the most popular channel, the most liked channel, and the most disliked channel
...
def get_most_popular_and_least_popular_channel(data):
    ...
    aggregate_channel = {}
    for item in data[1:]:                                               # the first one is title, so we should start by 1
        aggregate_channel.setdefault(item['channel_title'], 0)          # check, create keys and set 0 as default value
...
def get_most_liked_and_disliked_channel(data):
    ...
    for item in data[1:]:                                                           # the first one is title, so we should start by 1
        aggregate_channel_likes.setdefault(item['channel_title'], 0)                # check, create keys and set 0 as default value in aggregate_channel_likes dic
        aggregate_channel_likes[item['channel_title']] += int(item['likes'])        # update total likes of the corresponding channel
        aggregate_channel_dislikes.setdefault(item['channel_title'], 0)             # check, create keys and set 0 as default value in aggregate_channel_dislikes dic
